...
Chapters 0–8 are written. **Chapter 10 is not**: it is regenerated by `ml_meta_perf.report`
on every run, so the equation it analyses and the practices it states are always the ones
the current code produces. That separation is deliberate — the analysis of a fitted equation
should not depend on who reads it — and it is why the written chapters point at chapter 10
for numbers instead of copying tables that would go stale silently.
...
| 7 | [Limitations and threats to validity](07-limitations.md) — including the measured bound on better model descriptors | `ml_meta_perf.identity` |
| 8 | [Appendix: what was built, measured and rejected](08-appendix.md) | — |
