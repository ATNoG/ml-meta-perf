...
Chapters 1–9 are written. **Chapter 10 is not**: it is regenerated by `metafit.report` on
...
solve at every step. Three equations are contrasted — one over dataset features alone, one
over dataset and model features — to isolate how much of MCC is attributable to model
capability rather than dataset difficulty. All three are fitted on the same 476 rows by the
