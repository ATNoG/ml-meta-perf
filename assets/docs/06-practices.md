...
case in point: it appears as a factor, as a numerator, and in two denominators, with different
conditional directions. That is not the equation contradicting itself — it is the
...
which sums to exactly 1 even when groups are correlated. `Processing Units Number` varies
with dataset shape as well as the learner, and the selected terms also share raw features.
A naive $\operatorname{var}(g)/\operatorname{var}(\text{total})$
...
**The asserted descriptors carry a caveat wherever they are quoted.** Five of the six model
descriptors are assigned from published descriptions rather than measured from a run.
`Model Capability` needs particular care because its ordinal directly encodes a prior ranking,
so "higher capability rank went with higher MCC" is partly the ladder being read back out.
[Chapter 4](04-equation.md) sets out the costs in full.
...
general problem: the current `Processing Units Number` descriptor still varies with dataset
shape, and the mitigations below are still what stands between a fitted weight and a stated
practice.
...
That is why the generated section above states its practices at the level of received
...
- the marginal correlation is printed beside the conditional direction
  (the generated section above), so a reader can at least see when the two disagree, as they
  do here;
