...
compute it. **A practice can also be paired with the terms that carry it** — which terms,
with what standardised weight, and with which sign. That is in the generated section
...
two where it is a denominator. That is not the equation contradicting itself — it is the
...
10 practices assessed: 2 not tested, 2 qualified, 6 supported.
...
**Verdict: qualified.** The same equation scores R² 0.684 under a random 10-fold split and 0.691 when whole datasets are held out -- -0.007 of pure protocol. Dataset meta-features are constant within a dataset, so a random fold shows the equation rows from a dataset it is being scored on.
...
**Verdict: supported.** Noise-to-signal ratio moves predicted MCC by -0.95 between its lowest and highest decile, and it is one of only two features whose direction inside the equation agrees with its own correlation against MCC -- so it is not an artefact of conditioning. Nothing on the model side of the equation offsets it.
...
**Verdict: supported.** The highest-capacity family here is also the worst: generic neural networks average MCC 0.454 against 0.927 for tree ensembles. Capacity does not survive into the equation's terms in this run, so the family means are the whole of the evidence here.
...
**Verdict: qualified.** Tested against this study's own equation and the two cannot be separated -- which is the practice being right, since it claims the trivial baseline is competitive rather than that it wins. Ranking models within a held-out dataset, against the per-model-mean baseline -- ap 0.746 against 0.798, equation better on 5 of 18 datasets that differ, 95% CI [-0.107, +0.012]; mrr 0.825 against 0.835, equation better on 1 of 4 datasets that differ, 95% CI [-0.083, +0.065]; hit_at_1 0.750 against 0.750, equation better on 1 of 2 datasets that differ, 95% CI [-0.150, +0.150]; regret 0.008 against 0.011, equation better on 13 of 15 datasets that differ, 95% CI [-0.001, +0.008]. Every interval is a paired bootstrap over the twenty held-out datasets, because a difference of two means over twenty folds is not yet a measurement.
...
| When rows share a group -- a subject, a site, a dataset -- validate by holding out whole groups. A random split reports a number that will not survive deployment. | qualified | -0.0074 |
| Spend the first effort on reducing noise in the data, not on a larger model. Noise sets a ceiling that capacity cannot lift. | supported | -0.9515 |
...
| Before adopting a meta-learner to choose models, check it against 'use whatever usually works'. Ranking is an easier problem than prediction and often needs less. | qualified | -0.0516 |
...
The verdicts above are drawn from corpus averages -- family means, variance shares, paired tests -- which any study with this corpus could compute. This table asks the stronger question, and the one an interpretability-first study is uniquely able to ask: **which of the equation's terms carries this practice, with what strength and which sign?**

| practice | feature | expected | term | position | beta | effect | stability | rho | direction | agrees |
|---|---|---|---|---|---|---|---|---|---|---|
| clean-noise-before-adding-capacity | ns_ratio | lowers | ([log(class_ent)] + [log(ns_ratio)]) / [Loss Margin Behaviour] | numerator | -0.0803 | 0.1922 |  | -0.5813 | lowers | yes |
| clean-noise-before-adding-capacity | ns_ratio | lowers | ([log(eq_num_attr)] + [log(ns_ratio)]) / [log(nr_class)] | numerator | -0.2483 | 0.7178 |  | -0.8500 | lowers | yes |
| clean-noise-before-adding-capacity | ns_ratio | lowers | ([nr_cor_attr] + [log(ns_ratio)]) / [log(Processing Units Number)] | numerator | -0.0849 | 0.2139 | 0.0500 | -0.7668 | lowers | yes |
| clean-noise-before-adding-capacity | ns_ratio | lowers | ([log(ns_ratio)] + [log(Fitting Regime)]) / [log(eq_num_attr)] | numerator | 0.2064 | 0.3068 | 0.2000 | 0.5513 | raises | no |
| prefer-outlier-robust-learners | nr_outliers | lowers |  |  |  |  |  |  |  | not selected |
| capacity-is-not-free | Processing Units Number | lowers | [nr_cor_attr] / [log(Processing Units Number)] | denominator | -0.1315 | 0.3220 | 1.0000 | 0.5330 | raises | no |
| capacity-is-not-free | Processing Units Number | lowers | ([log(class_ent)] + [log(Processing Units Number)]) / [Fitting Regime] | numerator | -0.2809 | 0.7071 | 0.5000 | -0.6505 | lowers | yes |
| capacity-is-not-free | Processing Units Number | lowers | ([log(class_ent)] + [log(Processing Units Number)]) / [Loss Margin Behaviour] | numerator | 0.1763 | 0.4155 | 0.2000 | 0.8008 | raises | no |
| capacity-is-not-free | Processing Units Number | lowers | ([log(inst_to_attr)] + [log(Model Capability)]) / [log(Processing Units Number)] | denominator | 0.0598 | 0.1507 | 0.0500 | -0.8001 | lowers | yes |
| capacity-is-not-free | Processing Units Number | lowers | ([log(nr_class)] + [log(Loss Margin Behaviour)]) / [log(Processing Units Number)] | denominator | -0.1994 | 0.5036 | 0.2000 | 0.7936 | raises | no |
| capacity-is-not-free | Processing Units Number | lowers | ([nr_cor_attr] + [log(ns_ratio)]) / [log(Processing Units Number)] | denominator | -0.0849 | 0.2139 | 0.0500 | 0.4982 | raises | no |
| capacity-is-not-free | Model Capability | raises | [log(Fitting Regime)] * [log(Model Capability)] | factor | -0.1733 | 0.3729 | 0.1500 | -0.7794 | lowers | no |
| capacity-is-not-free | Model Capability | raises | [log(eq_num_attr)] * [log(Model Capability)] | factor | 0.0990 | 0.2527 | 0.8500 | 0.6720 | raises | yes |
| capacity-is-not-free | Model Capability | raises | [log(gravity)] * [log(Model Capability)] | factor | 0.2682 | 0.6886 | 1.0000 | 0.5408 | raises | yes |
| capacity-is-not-free | Model Capability | raises | [nr_norm] * [log(Model Capability)] | factor | 0.0474 | 0.0921 | 0.3500 | 0.3220 | raises | yes |
| capacity-is-not-free | Model Capability | raises | ([log(inst_to_attr)] + [log(Model Capability)]) / [log(Processing Units Number)] | numerator | 0.0598 | 0.1507 | 0.0500 | -0.3045 | lowers | no |
| capacity-is-not-free | Model Capability | raises | ([log(Loss Margin Behaviour)] + [log(Model Capability)]) / [Fitting Regime] | numerator | -0.0351 | 0.0941 |  | -0.3664 | lowers | no |
...
**Read this against the verdict tally above, not as part of it.** Of the 10 practices, 3 make a claim about a quantity the equation contains; the rest are about a protocol, a metric, or a family of learners, and pairing one of those with a coefficient would be inventing a connection. Those are carried by **14 of the equation's 25 terms**, giving 16 directed (practice, term) pairings: **8 come out the way the practice predicts and 8 do not**, with 0 pairing too weak to state a direction for and 1 claim resting on a feature the search never took.
...
**These are associations across 20 datasets, not causal claims, and not practices on their own** — a statement about a meta-feature column is a measurement. Section 5 is where they become advice, by supporting or failing to support something a practitioner could already have been told.
 ...
 1. [unrated ] Higher noise-to-signal ratio went with lower MCC (about 0.95 MCC between its lowest and highest decile).
 2. [unrated ] Higher how the parameters are reached, closed form to in-context (1-5) went with lower MCC (about 0.68 MCC between its lowest and highest decile).
 3. [unrated ] Higher how hard the loss penalises points far from the boundary (1-5) went with lower MCC (about 0.59 MCC between its lowest and highest decile).
 4. [unrated ] Higher gravity (separation between the majority and minority class centres) went with higher MCC (about 0.21 MCC between its lowest and highest decile).
 5. [unrated ] Higher number of classes went with lower MCC (about 0.05 MCC between its lowest and highest decile).

Evidence:

| feature | meaning | n_terms | direction | effect | stability | confidence |
|---|---|---|---|---|---|---|
| ns_ratio | noise-to-signal ratio | 4 | -0.6708 | -0.9515 |  | unrated |
| Fitting Regime | how the parameters are reached, closed form to in-context (1-5) | 7 | -0.2250 | -0.6837 |  | unrated |
| Loss Margin Behaviour | how hard the loss penalises points far from the boundary (1-5) | 8 | -0.5384 | -0.5905 |  | unrated |
| gravity | gravity (separation between the majority and minority class centres) | 4 | 0.2227 | 0.2135 |  | unrated |
| nr_class | number of classes | 5 | -0.2633 | -0.0466 |  | unrated |

#### These are conditional statements, not marginal ones

A practice states what the *equation* does as a feature rises, with every other term present. A marginal correlation states what the feature does alone. They are different quantities, and here **3 of 5 agree** on the sign:

| feature | meaning | marginal | conditional | practice_says | agrees |
|---|---|---|---|---|---|
| ns_ratio | noise-to-signal ratio | -0.3954 | -0.6708 | lower | yes |
| Fitting Regime | how the parameters are reached, closed form to in-context (1-5) | 0.0502 | -0.2250 | lower | no |
| Loss Margin Behaviour | how hard the loss penalises points far from the boundary (1-5) | -0.1838 | -0.5384 | lower | yes |
| gravity | gravity (separation between the majority and minority class centres) | -0.2863 | 0.2227 | higher | no |
| nr_class | number of classes | -0.0726 | -0.2633 | lower | yes |
...
```
dataset  : DDOS-ANL
model    : LightGBM_ExtraTrees
actual   : +0.9256
predicted: +1.0000

contribution breakdown:
    +2.0261   intercept
    -0.5705   ([log(class_ent)] + [log(Processing Units Number)]) / [Fitting Regime]
    -0.4988   [log(inst_to_attr)] * [log(nr_class)]
    +0.4607   [log(gravity)] * [log(Model Capability)]
    +0.3577   ([log(ns_ratio)] + [log(Fitting Regime)]) / [log(eq_num_attr)]
    -0.3415   ([log(eq_num_attr)] + [log(gravity)]) / [log(nr_inst)]
    -0.2939   ([log(class_ent)] + [log(inst_to_attr)]) / [log(eq_num_attr)]
    -0.1356   the remaining 19 terms
  = +1.0042   sum
