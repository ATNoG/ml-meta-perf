...
Meta-dataset: `C:\Datos\IT projects\6 Framework Cost of ML\ml-meta-perf\src\ml_meta_perf\meta_dataset.csv`
...
**These are associations across 20 datasets, not causal claims, and not practices on their own** — a statement about a meta-feature column is a measurement. Section 5 is where they become advice, by supporting or failing to support something a practitioner could already have been told.
 ...
 2. [moderate] Higher learner family's capability rank in the tabular-ML literature (1-10) went with higher MCC (about 0.38 MCC between its lowest and highest decile).
 3. [moderate] Higher model capacity (log processing units) went with higher MCC (about 0.33 MCC between its lowest and highest decile).
 ...
 7. [unrated ] Higher how hard the loss penalises points far from the boundary (1-5) went with lower MCC (about 0.06 MCC between its lowest and highest decile).
 8. [weak    ] Higher number of classes went with lower MCC (about 0.05 MCC between its lowest and highest decile).

Evidence:

| feature | meaning | n_terms | direction | effect | stability | confidence |
|---|---|---|---|---|---|---|
| eq_num_attr | equivalent number of attributes (effective feature count) | 3 | -0.7375 | -0.5834 | 0.8167 | moderate |
| Model Capability | learner family's capability rank in the tabular-ML literature (1-10) | 3 | 0.7123 | 0.3774 | 0.8000 | moderate |
| Processing Units Number | model capacity (log processing units) | 5 | 0.3607 | 0.3273 | 0.6600 | moderate |
| gravity | gravity (separation between the majority and minority class centres) | 5 | -0.5039 | -0.3241 | 0.5700 | moderate |
| nr_cor_attr | proportion of correlated attribute pairs | 2 | -0.7214 | -0.1769 |  | unrated |
| nr_bin | number of binary attributes | 1 | 0.8439 | 0.1525 | 0.5000 | moderate |
| Loss Margin Behaviour | how hard the loss penalises points far from the boundary (1-5) | 1 | -0.5913 | -0.0603 |  | unrated |
| nr_class | number of classes | 3 | -0.4126 | -0.0467 | 0.5500 | weak |
