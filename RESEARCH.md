...
leave-one-dataset-out R² of ~0.37 for a classification metric (MCC), against an in-sample
0.556, is not a failure of the method — it reflects a documented property of classifier
...
grouped split. The identical equation scores 0.514 under random 10-fold and 0.371 under
...
**additive oracle ceiling is 0.661**: with exact per-dataset and per-model effects, no
additive model can exceed that on this data. A reported 0.7 therefore implies either
non-additive structure or a leaky split.
...
| Ceiling stated | rarely | additive oracle at 0.661, E1 capped at 0.354 |
...
- **More datasets.** Twenty is the binding constraint on every cross-validated number
  here; OpenML-scale meta-data would settle whether the 0.661 additive ceiling is a
  property of this sample or of the approach.
- **Interaction structure.** The gap between E2 (0.556) and the additive oracle (0.661)
