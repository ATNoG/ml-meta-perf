"""Choosing which terms enter the equation.

This is the search. `ml_meta_perf.fit` is the arithmetic it scores candidates with, and the
split is the study's own: a form is chosen once, and weights are refitted per fold against a
form held still -- which is what `validate.cross_validate_fixed_form` reports. Until
2026-09-09 both lived in a module called `fit`, whose entry point was a function called `fit`
that ran a beam search, and the one distinction the results rest on was invisible in the names.
...
*Pruning is a decision about form, so it is here.* `prune` drops terms that do no work and
simplifies what remains -- both changes to which terms the equation contains -- and then asks
`fit` for the weights of what is left. An equation shortened without that refit would be the
best fit of terms it no longer has.

**`Selector` holds a ridge fit and is still in this module.** Its `_evaluate` is arithmetic
that `fit` would recognise, but it exists to be called several hundred thousand times by the
beam, off a Gram matrix and a cache the beam's constructor builds. Splitting it would put the
