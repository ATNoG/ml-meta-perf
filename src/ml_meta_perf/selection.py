...
**The rule is now `best_length`: the argmax of the consensus curve.** It has no threshold, no
smoothing window and no sensitivity parameter, so it is a property of the curve rather than of
a value chosen to produce a preferred answer, and it re-derives itself when the corpus changes.
The same rule serves both equations the study publishes, under the two grammars it reports:
15 terms under the parsimonious grammar (arity 2) and 23 under the full one (arity 3).
